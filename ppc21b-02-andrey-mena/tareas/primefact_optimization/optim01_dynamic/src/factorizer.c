...
void direct_threads(pthread_t* threads , shared_data_t* sharedData,
  ...
  int error = EXIT_SUCCESS;
  sharedData->positionToRead = sharedData->threadCount-1;
  for (uint64_t threadNumber = 0; threadNumber < sharedData->threadCount
    ; threadNumber++) {
    privateData[threadNumber].threadNumber = threadNumber;
    privateData[threadNumber].positionInTheArray = threadNumber;
    privateData[threadNumber].integer =
      sharedData->numbers.elements[threadNumber];
...
void* factorize_all_numbers(void* data) {
  ...
  while (salir == 0) {
    ...
    sharedData->positionToRead++;
    privateData->positionInTheArray = sharedData->positionToRead;
    if (sharedData->positionToRead >= sharedData->numbers.size) {
      salir = 1;
    } else {
      privateData->integer =
